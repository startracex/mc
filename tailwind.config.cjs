...
fs.readdirSync(`./public/${BLOCK_DIR}`).forEach(file => {
	...
	background_obj[block_name] = `url('${baseurl}${BLOCK_DIR}/${file}')`
...
fs.readdirSync(`./public/${ITEM_DIR}`).forEach(file => {
	...
	background_obj[item_name] = `url('${baseurl}${ITEM_DIR}/${file}')`
...
fs.readdirSync(`./public/${PAINTING_DIR}`).forEach(file => {
	...
	background_obj[painting_name] = `url('${baseurl}${PAINTING_DIR}/${file}')`
